...
import {
  Globe, Search, Rss, MessageSquare, TrendingUp,
...
function MetricCard({ icon: Icon, label, value, sub, accent = false }: {
  ...
  return (
    <motion.div
      ...
      className="relative panel overflow-hidden cursor-default group"
    >
      {/* Top accent line */}
      {accent && (
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([api.getStats(), api.getItems({ limit: 10 })])
  ...
  const searches   = stats?.total_searches ?? stats?.search_sessions_total ?? 0
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      ...
      <motion.div variants={STAGGER} initial="hidden" animate="show"
        ...
        <MetricCard icon={Globe}         label="Browses"          value={browses}                                    accent />
        <MetricCard icon={Search}        label="Recherches"       value={searches} />
        <MetricCard icon={Rss}           label="Items"            value={totalItems} sub={`${stats?.classified_items ?? 0} classifiés`} />
        <MetricCard icon={Radio}         label="Surveillées"      value={stats?.watched_pages ?? 0}                  accent />
        <MetricCard icon={TrendingUp}    label="Digests"          value={stats?.digests_generated ?? 0} />
        <MetricCard icon={MessageSquare} label="Queries RAG"      value={stats?.rag_queries_total ?? 0} />
        <MetricCard icon={Clock}         label="Coût LLM / mois"  value={`$${cost.toFixed(3)}`} />
        <MetricCard icon={Cpu}           label="Outils MCP"       value="20" sub="actifs" accent />
      </motion.div>
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className={`w-1 h-5 rounded-full flex-shrink-0 ${
                            it.importance === 'critical' ? 'bg-red-500' :
                            it.importance === 'high'     ? 'bg-orange-500' :
                            it.importance === 'medium'   ? 'bg-yellow-500' : 'bg-[hsl(var(--bg-3))]'
                          }`} />
