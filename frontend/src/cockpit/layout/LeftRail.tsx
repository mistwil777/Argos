...
import { FileText, BookOpen, MessageSquare, Rss, BarChart2, Home, Zap, Terminal } from 'lucide-react';
...
export function LeftRail() {
  ...
  return (
    <nav className="w-[72px] shrink-0 bg-zinc-950 border-r border-white/[0.05] flex flex-col items-center pt-3 pb-4">
      ...
      {/* Admin DevOps — séparé en bas */}
      <div className="w-8 h-px bg-white/[0.05] mb-2 shrink-0" />
      <button
        onClick={() => setActiveMode('devops')}
        title="DevOps — Admin"
        className={`
          relative flex flex-col items-center gap-1 w-full py-2.5 px-1
          transition-colors duration-150 group
          ${isDevOps ? 'text-amber-400' : 'text-zinc-700 hover:text-amber-500/70'}
        `}
      >
        <div className={`
          absolute left-0 top-2.5 bottom-2.5 w-[3px] rounded-r-full transition-all duration-200
          ${isDevOps ? 'bg-amber-500 opacity-100' : 'opacity-0'}
        `} />
        <div className={`
          w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-150
          ${isDevOps ? 'bg-amber-500/12' : 'group-hover:bg-amber-500/[0.06]'}
        `}>
          <Terminal className="w-[17px] h-[17px]" strokeWidth={isDevOps ? 2 : 1.5} />
        </div>
        <span className={`
          text-[9px] font-medium tracking-wide leading-none transition-colors duration-150
          ${isDevOps ? 'text-amber-400' : 'text-zinc-800 group-hover:text-amber-500/60'}
        `}>
          DevOps
        </span>
      </button>
